...
(function(){
  ...
  const TEAM_ALIASES={'Premature Ejelculators':'Premature Ejleculators','braddydrinksthebud':'Premature Ejleculators'};
  ...
  function mergeData(old={},patch={}){
    ...
    if(Array.isArray(patch.transactions)&&patch.transactions.length){const merged=mergeKeyed(old.transactions,patch.transactions,transactionIdentity),groups={};for(const x of merged){const g=(x.team&&x.timestamp)?(x.team+'|'+x.timestamp):('raw|'+transactionIdentity(x));(groups[g]||(groups[g]=[])).push(x)}const compact=[];for(const items of Object.values(groups)){const rich=items.filter(x=>x.type&&x.timestamp&&((x.added||[]).length||(x.dropped||[]).length||x.type==='TRADE'));const src=rich.length?rich:items;const uniq={};for(const x of src){const ps=(x.players||[]).map(p=>String(p.name||'').toLowerCase()+':'+norm(p.action||'')).sort().join(',');const mv=[...(x.added||[]).map(p=>'+'+String(p.name||'').toLowerCase()),...(x.dropped||[]).map(p=>'-'+String(p.name||'').toLowerCase())].sort().join(',');const k=[x.team||'',x.timestamp||'',x.type||'',ps||mv,x.faabSpent??''].join('|');if(!uniq[k])uniq[k]=x}compact.push(...Object.values(uniq))}o.transactions=compact;}
  ...
  function setMode(){state.mode='WEDNESDAY';saveState()}
