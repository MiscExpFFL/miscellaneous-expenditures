(function(){
  ...
  function normalizeTx(tx){
    const add=tx?.add!=null?String(tx.add):(tx?.added||[]).map(p=>p?.name||p).filter(Boolean).join(', ');
    const drop=tx?.drop!=null?String(tx.drop):(tx?.dropped||[]).map(p=>p?.name||p).filter(Boolean).join(', ');
    const faab=tx?.faab!=null?num(tx.faab):(tx?.faabSpent!=null?num(tx.faabSpent):null);
    const time=tx?.time||tx?.timestamp||tx?.date||'';
    return {...tx,add,drop,faab,time,description:tx?.description||[add?('Added '+add):'',drop?('Dropped '+drop):''].filter(Boolean).join(' · ')};
  }
  function txEvents(){
  ...
  function renderLuck(){
    ...
    return section('luck','LUCK & EFFICIENCY','Schedule luck, all-play truth and scoring context',body,'')
  }
  ...
  function renderFrontOffice(){
    ...
    const body=`<div class="analytics-stat-grid">${statCard('Front-office leader',top?`${top.manager} · ${top.grade}`:'Awaiting Week 1','Grades activate when acquired players produce scored weeks')}${statCard('Best acquisition',bestAdd?`${bestAdd.bestAdd.add} · ${bestAdd.bestAdd.impact.toFixed(2)} pts`:'Awaiting Week 1',bestAdd?`${bestAdd.manager} · from Week ${bestAdd.bestAdd.startWeek}`:'The acquisition tracker is already armed')}${statCard('Worst FAAB burn',worstBurn?`${worstBurn.add} · $${worstBurn.faab}`:'Not enough scored data',worstBurn?`${worstBurn.impact.toFixed(2)} points after acquisition`:'Activates after paid claims produce points')}${statCard('Moves logged',String(rows.reduce((s,x)=>s+x.moves,0)),'2026 deduplicated manager activity')}${statCard('FAAB tied to adds',`$${rows.reduce((s,x)=>s+x.faab,0).toFixed(0)}`,'Public completed spending only')}</div>`+
      ...
      `<div class="analytics-subsection"><h3>Historical activity efficiency</h3>${table(['Manager','Career moves','Regular-season wins','Wins / 100 moves'],historical.map(r=>`<tr><td><b>${e(r.manager)}</b></td><td>${r.moves}</td><td>${r.w}</td><td>${r.per100==null?'—':r.per100.toFixed(1)}</td></tr>`).join(''))}</div>`;
    return section('front-office','FRONT OFFICE','Transaction ROI & front-office grades',body,'')
  }
  ...
  function renderDNAGrid(){
    ...
    return section('dna','MANAGER DNA','How every manager actually operates',`<div class="dna-grid">${cards}</div>`,'')
  }
  ...
  function renderPulse(){
    ...
    return section('pulse','LEAGUE PULSE','Season timeline & weekly pulse',kpis+weekCards+timeline,'')
  }
  ...
  function renderAwards(innerOnly=false){
    ...
    const board=`<div class="analytics-stat-grid awards-stats">${statCard('Awards logged',String(total),'Published weekly awards')}${statCard('Awards leader',top&&top.awards?`${top.manager} · ${top.awards}`:'Week 1 board','Manager credits from named recipients')}${statCard('GOTW appearances',gotw&&gotw.gotw?`${gotw.manager} · ${gotw.gotw}`:'Week 1 board','Both managers receive an appearance')}${statCard('Published award weeks',String(new Set(a.archive.map(x=>x.week)).size),'Archive depth')}</div>`+
      ...
      `<div class="analytics-subsection weekly-award-archive"><div class="award-archive-heading"><div><span>THE RECEIPTS</span><h3>Weekly award archive</h3></div><small>${a.archive.length?`${a.archive.length} award${a.archive.length===1?'':'s'} filed`:'No awards filed yet'}</small></div><div class="award-archive-grid">${a.archive.length?a.archive.map(x=>`<article class="award-receipt"><div class="award-receipt-top"><span>WEEK ${x.week}</span><em>AWARD RECEIPT</em></div><h4>${e(x.name)}</h4><div class="award-recipient">${e(x.recipient||'')}</div><p>${e(x.detail||`Filed with the Week ${x.week} write-up.`)}</p><a href="week.html?week=${x.week}">Open Week ${x.week} write-up →</a></article>`).join(''):'<div class="analytics-empty compact">No weekly awards have been published yet.</div>'}</div></div>`;
    return innerOnly?board:section('awards','WEEKLY HARDWARE','Weekly Awards Leaderboard',board,'')
  }
